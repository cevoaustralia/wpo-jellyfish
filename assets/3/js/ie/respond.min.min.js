...
return t.style.cssText="position:absolute;font-size:1em;width:1em",n||(n=s=d.createElement("body"),n.style.background="none"),h.style.fontSize="100%",n.style.fontSize="100%",n.appendChild(t),s&&h.insertBefore(n,h.firstChild),e=t.offsetWidth,s?h.removeChild(n):n.removeChild(t),h.style.fontSize=a,r&&(n.style.fontSize=r),e=m=parseFloat(e)},E=function(t){var n="clientWidth",a=h[n],r="CSS1Compat"===d.compatMode&&a||d.body[n]||a,s={},i=x[x.length-1],p=(new Date).getTime()
if(t&&o&&30>p-o)return e.clearTimeout(l),void(l=e.setTimeout(E,30))
o=p
for(var y in u)if(u.hasOwnProperty(y)){var w=u[y],S=w.minw,T=w.maxw,C=null===S,b=null===T
S&&(S=parseFloat(S)*(S.indexOf("em")>-1?m||v():1)),T&&(T=parseFloat(T)*(T.indexOf("em")>-1?m||v():1)),w.hasquery&&(C&&b||!(C||r>=S)||!(b||T>=r))||(s[w.media]||(s[w.media]=[]),s[w.media].push(c[w.rules]))}for(var $ in f)f.hasOwnProperty($)&&f[$]&&f[$].parentNode===g&&g.removeChild(f[$])
f.length=0
for(var z in s)if(s.hasOwnProperty(z)){var M=d.createElement("style"),R=s[z].join("\n")
M.type="text/css",M.media=z,g.insertBefore(M,i.nextSibling),M.styleSheet?M.styleSheet.cssText=R:M.appendChild(d.createTextNode(R)),f.push(M)}},w=function(e,t,a){var r=e.replace(n.regex.comments,"").replace(n.regex.keyframes,"").match(n.regex.media),s=r&&r.length||0
...
for(var g=0;p>g;g++)h=f[g],i(h)||u.push({media:h.split("(")[0].match(n.regex.only)&&RegExp.$2||"all",rules:c.length-1,hasquery:h.indexOf("(")>-1,minw:h.match(n.regex.minw)&&parseFloat(RegExp.$1)+(RegExp.$2||""),maxw:h.match(n.regex.maxw)&&parseFloat(RegExp.$1)+(RegExp.$2||"")})}E()},S=function(){if(a.length){var t=a.shift()
s(t.href,function(n){w(n,t.href,t.media),p[t.href]=!0,e.setTimeout(function(){S()},0)})}},T=function(){for(var t=0;t<x.length;t++){var n=x[t],r=n.href,s=n.media,i=n.rel&&"stylesheet"===n.rel.toLowerCase()
r&&i&&!p[r]&&(n.styleSheet&&n.styleSheet.rawCssText?(w(n.styleSheet.rawCssText,r,s),p[r]=!0):(!/^([a-zA-Z:]*\/\/)/.test(r)&&!y||r.replace(RegExp.$1,"").split("/")[0]===e.location.host)&&("//"===r.substring(0,2)&&(r=e.location.protocol+r),a.push({href:r,media:s})))}S()}
